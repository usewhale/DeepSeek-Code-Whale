...
// webSearchCallRegistry remembers web_search_call items per session so they
// can be echoed back on later turns (the Responses API is stateless; echoing
// the items lets the server restore search results without re-running the
// search). The registry is in-memory only: after a restart the items are
// missing and the server simply re-searches, which stays correct.
type webSearchCallRegistry struct {
	mu        sync.Mutex
	bySession map[string][]map[string]any
}

func (r *webSearchCallRegistry) record(sessionID string, items []map[string]any) {
	if r == nil || strings.TrimSpace(sessionID) == "" || len(items) == 0 {
		return
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.bySession == nil {
		r.bySession = map[string][]map[string]any{}
	}
	r.bySession[sessionID] = append(r.bySession[sessionID], items...)
}

func (r *webSearchCallRegistry) lookup(sessionID string) []map[string]any {
	if r == nil || strings.TrimSpace(sessionID) == "" {
		return nil
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	items := r.bySession[sessionID]
	...
	case "response.completed", "response.incomplete", "response.failed":
		...
		if registry != nil && sessionID != "" {
			if items := responsesWebSearchCallItems(resp.Output); len(items) > 0 {
				registry.record(sessionID, items)
			}
		}
